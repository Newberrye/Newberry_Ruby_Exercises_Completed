def find_language_information(languages, language_name)
  ...
  # }
    name = languages.fetch(language_name, {})
end
...
def add_information_about_language(languages, language_name, info_key, info_value)
  ...
  newHash = {}
  newHash[info_key] = info_value
  languages[language_name].merge!(newHash)
  languages
end
...
def add_language(languages, language_name, language_info_value)
  ...
  newHash = {}
  newHash[language_name] = language_info_value
  languages.merge!(newHash)
  languages
end
...
def find_beautiful_languages(languages)
  ...
  languages.each do |key,value|
    if value == true
      return hash.merge(languages[key])
    end
  hash  
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  #                 is_beautiful?: false }
  # }
  languages.dig(language_name).fetch(:facts)[fact_index]
end
